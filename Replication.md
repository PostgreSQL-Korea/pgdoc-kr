...
2. 주 서버 사전 준비  
   보조 서버를 pg\_basebackup 명령으로 구축하려면, 먼저 주 서버의 postgresql.conf 파일과 pg\_hba.conf파일에 다음 설정을 해야 한다. 또한 여기서 구축하는 방법에서는 복제 슬롯을 이용해서 주 서버가 보관해야할 아직 전송하지 않은 WAL 를 모두 보관하는 방식을 택했다. 또한 고가용성을 고려해서 pg_rewind 명령을 사용하기 위해서, 초기 주 서버는 데이터 클러스터를 만들 때 체크섬 기능을 켰다(initdb -k 옵션).
...
1. 개요
   PostgreSQL 10 버전부터 논리적 디코딩을 이용한 논리적 복제 기능을 제공한다.

   현재 논리적 복제는 테이블 단위이며, 아직까지는(ver. 10 beta3) 복제 과정에서 생기는 자료 충돌 문제는 사용자가 직접 풀어야 한다.

   논리적 복제는 다음과 같은 제약이 있다.

   * 복제는 테이블 단위로 이루워진다. 시퀀스, 함수 등 다른 객체들에 대한 복제는 제공하지 않는다.
   * 그 테이블은 이미 복제 대상 서버에 원본과 똑같은 구조로 있어야 한다.
   * 자료 충돌 문제(이미 자료가 있거나, 교차 참조 순환에 빠지거나)가 발생하는 경우, 그 트랜잭션 이하 모든 트랜잭션은 더 이상 복제를 진행하지 않는다.  충돌 해결 옵션이 없다.

   논리적 복제는 발행과 구독이라는 개념을 사용한다.  주 서버에서 해당 테이블을 (변경된 내역을) 발행하고, 그 발행한 내용을 구독하는 쪽에서 그것을 자신의 서버에 반영하는 방식이다.

   논리적 복제에 대한 이하 모든 설명은 PostgreSQL 10 버전 이상에 대해서만 해당하는 부분이다.
...
2. 발행
   ...
   발행 서버의 wal_level 값은 logical 이어야 한다.


   ```
   postgresql@postgres=# create publication replica_a for table a ;
   CREATE PUBLICATION
   ```

   발행 옵션으로 inserts, updates, deletes 가 있으며, 각 DML에 대해서만 발행할 수 있다.

3. 구독
   ```
   postgresql@postgres=# create subscription subscript_a 
   postgres-# connection 'host=localhost user=postgres dbname=postgres port=5432' publication replica_a;
   CREATE SUBSCRIPTION
   ```

   간단하다!
   너무 간단해서 구독의 기능이 많이 미흡하기는 하다.

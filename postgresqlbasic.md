...
현재 ON SELECT 규칙은 무조건 INSTEAD 규칙이어야하며 단일 SELECT 명령으로 구성된 작업이 있어야합니다. 따라서 ON SELECT 규칙은 테이블을 뷰로 효과적으로 바꿉니다.이 뷰의 내용은 테이블에 저장된 모든 것이 아닌 규칙의 SELECT 명령에 의해 반환 된 행입니다. 실제 테이블을 만들고 그것에 대한 ON SELECT 규칙을 정의하는 것보다 CREATE VIEW 명령을 작성하는 것이 더 좋은 스타일로 간주됩니다.

ON INSERT, ON UPDATE 및 ON DELETE 규칙 \(또는 목적에 부합하는 하위 집합\)을 정의하여 뷰의 업데이트 동작을 다른 테이블의 적절한 업데이트로 대체하여

업데이트 가능한 뷰의 환상을 만들 수 있습니다. INSERT RETURNING 등을 지원하려면 이러한 각 규칙에 적절한 RETURNING 절을 넣어야합니다.

복잡한보기 업데이트에 조건부 규칙을 사용하려고하면 catch가 발생합니다.보기에서 허용하려는 각 작업에 대해 무조건 INSTEAD 규칙이 있어야합니다. 규칙이

조건부이거나 INSTEAD가 아닌 경우 시스템은 경우에 따라보기의 더미 테이블에서 작업을 수행하려고 시도 할 수 있다고 생각하기 때문에 업데이트 작업 수행 시도를 거부합니다. 조건부 규칙에서 유용한 모든 경우를 처리하려면 무조건 DO INSTEAD NOTHING 규칙을 추가하여 더미 테이블을 업데이트하라는 절대로 시스템이 인식하지 못하도록합니다. 그런 다음 조건부 규칙을 INSTEAD가 아닌 상태로 만듭니다. 적용되는 경우 기본 INSTEAD NOTHING 동작에 추가됩니다. 그러나이 메서드는 현재 RETURNING 쿼리를 지원하지 않습니다.

...
어렵다...................... 이해를 하고 작성해야 하는데...... 엄청 어렵네.. ㅠ.ㅠ
...
검색 속도를 높이기 위해 사용하는 일반적인 방법.

인덱스를 추가하면 매번 행이 추가되거나 변경될 때마다 오버헤드가 증가하기 때문에 인덱스가 효율적인지 판단이 필요함.

1\) 소개

인덱스는 Create, Drop 문을 사용하여 생성 삭제할 수 있다.

생성 후 추가 변환은 필요 없으며, 조인 검색시 조인 조건 컬럼에서도 인덱스를 활용하여 쿼리 속도를 높힐 수 있다.

인덱스 생성과 병행하여 테이블 읽기\(SELECT\)는 허용하지만 쓰기\(INSERT, UPDATE, DELETE\)는 허용하지 않음.

2\) 인덱스 타입

B-트리, 해시, Gist, SP\_Gist, GIN 을 제공함

가. B-트리

일반적인 인덱스 타입은 B-트리이며, B는 균형\(balanced\)를 의미함

왼쪽, 오른쪽 비슷한 양의 데이터를 균등하게 보관하며 어떤 데이터에 접근하더라도 비슷한 레벨의 접근이 가능함

단일값, &gt;, &gt;=, =, &lt;, &lt;= 등의 연산자가 사용될 경우 B-트리 인덱스를 사용하는 것으로 간주
